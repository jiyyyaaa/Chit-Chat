...
const ProfilePage = () => {
    ...
    const handleSubmit= async (e)=>{
        e.preventDefault();
        if (!selectedImg){
            await updateProfile({ProfilePic: authUser?.ProfilePic, fullName: name, bio});
            navigate('/');
            return;
        }
  ...
  return (
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
        <div className='w-5/6 ax-w-2xl backdrop-blur-2xl text-gray-300 border-2
        ...
        rounded-lg'>
            <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
                ...
                to-violet-600 text-white p-2rounded-full text-lg cursor-pointer'>Save</button>
            </form>
